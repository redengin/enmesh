...
use super::interface::Ssd1680Interface;


pub struct Screen<SPI, BSY, RST, DC, DLY> {
    interface: Ssd1680Interface<SPI, BSY, RST, DC>,
    delay: DLY,
    // dimensions: Size,  122x250
}

impl<SPI, BSY, DC, RST, DLY> Screen<SPI, BSY, DC, RST, DLY>
where
    SPI: SpiDevice,
    RST: OutputPin,
    DC: OutputPin,
    ...
    DLY: DelayNs,
{
    ...
    where
        Self: Sized,
    {
        let interface = Ssd1680Interface::new(spi, busy, dc, rst);
        let mut this = Self { interface, delay };
        // this.init(delay)?;
        Ok(this)
    }
